...
def enableCanSignInArcGIS(portalUrl,token):
    ...
    try:
        request = Request(portalUrl + '/sharing/rest/portals/self?token=' + token + '&f=json')
        ...
        if 'error' in portalSelf:
            ...
            if 'canSignInArcGIS' in portalSelf:
                if portalSelf.get('canSignInArcGIS'):
                    ...
                    params = {'canSigninArcGIS':'true'}
                    request = Request(portalUrl + '/sharing/rest/portals/0123456789ABCDEF/update?token=' + token + '&f=json', urlencode(params).encode())
                    request.add_header('Referer', 'canSignInArcGIS')
                    response = urlopen(request)
